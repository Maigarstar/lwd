...
-- Safe to re-run: WHERE NOT EXISTS guards against duplicates (no unique constraint needed)
...
BEGIN
  ...
  WHERE NOT EXISTS (
    ...
    WHERE reviewer_email = 'isabelle.moreau@lwd-seed.example'
  ...
  WHERE NOT EXISTS (
    SELECT 1 FROM reviews
    WHERE reviewer_email = 'charlotte.ashworth@lwd-seed.example'
      AND entity_id = v_listing_id
  );
  ...
  WHERE NOT EXISTS (
    SELECT 1 FROM reviews
    WHERE reviewer_email = 'priya.mehta@lwd-seed.example'
      AND entity_id = v_listing_id
  );
