 ...
 * Reads go through the edge function (service_role) so RLS never blocks them.
...
export async function fetchManagedAccounts(opts = {}) {
  try {
    ...
    const rows = (Array.isArray(data) ? data : []).map(dbToAccount);
    console.debug('[SocialStudio] fetchManagedAccounts — DB returned', rows.length, 'rows');
    return rows;
  } catch (err) {
...
export async function fetchManagedAccount(id) {
  try {
    const data = await callEdge({ action: 'getById', id });
