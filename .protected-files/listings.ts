 ...
 * Routes through the admin-listings edge function (service_role) so RLS never
 * blocks draft/unpublished listings in the admin context.
 */
export async function fetchListingById(id: string) {
  try {
    const data = await callListingsEdge({ action: 'getById', id })
    if (!data) throw new Error(`Listing not found: ${id}`)
...
/**
 * Admin alias: fetch all listings regardless of status.
 * Routes through edge function (service_role) so drafts are always visible.
 */
...
/**
 * Admin alias: fetch a single listing by slug bypassing RLS.
 * Routes through edge function (service_role) so draft listings are visible.
 */
export async function fetchListingBySlugAdmin(slug: string) {
  try {
    const data = await callListingsEdge({ action: 'getBySlug', slug })
    if (!data) return null
    return mapListingFromDb(data)
  } catch (error) {
    console.error('Error fetching listing by slug (admin):', error)
    return null
  }
}
...
export async function fetchListings(filters?: {
  ...
  try {
    // Route through edge function (service_role) so RLS never hides drafts in admin
    const data = await callListingsEdge({ action: 'list', filters: filters ?? {} })
    const rows: any[] = Array.isArray(data) ? data : []
    const camelListings = rows.map(listing => snakeToCamel(listing))
    return camelListings.map(listing => transformSupabaseListingForUI(listing)) as Listing[]
  } catch (error) {
