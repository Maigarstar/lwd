...
// Route all listings reads/writes through the admin edge function (service_role)
async function invokeAdminListings(action, params = {}) {
  const { data, error } = await supabase.functions.invoke('admin-listings', {
    body: { action, ...params },
    headers: { Authorization: `Bearer ${ANON_KEY}` },
  });
  if (error) throw new Error(error.message || 'admin-listings error');
...
export async function generateListingSeo(listing) {
  ...
  // Patch via admin edge function (service_role bypasses RLS)
  const result = await invokeAdminListings('update', {
    id:      listing.id,
    payload: { seo_title: seoTitle, seo_description: seoDescription, seo_keywords: seoKeywords },
  });
