...
// Listings writes go through the admin edge function (service_role bypasses RLS)
async function updateListingViaEdgeFn(id, payload) {
  const { data, error } = await supabase.functions.invoke('admin-listings', {
    body: { action: 'update', id, payload },
  });
  if (error) throw error;
  if (!data?.success) throw new Error(data?.error ?? 'admin-listings/update failed');
}
  ...
  for (const action of actions) {
    try {
      if (action === 'activate_profile' && prospect.listing_id) {
        ...
        await updateListingViaEdgeFn(prospect.listing_id, { status: 'active' });
