...
const BUDGET_OPTIONS = ['Under 10k', '10k-20k', '20k-50k', '50k+'];
...
export default function VendorEnquiryForm({
  ...
  const [form, setForm] = useState({
    message: `Hi${vendorName ? ' ' + vendorName : ''}, I'd love to learn more about your services for my upcoming wedding. Could you share pricing and availability?`,
    firstName: '',
    lastName: '',
    email: '',
    phone: '',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await createLead({
        ...
        listingId: listingId || vendorId || null,
        listingType: vendorCategory || 'vendor',
        ...
        email: form.email,
        phone: form.phone,
        preferredContactMethod: form.preferredContact || undefined,
      ...
      console.error('VendorEnquiryForm: submit failed:', err);
      ...
      <div style={containerStyle}>
        <div style={S.successWrap}>
          ...
          <p style={S.successText}>
            Thank you! We've sent your enquiry to {vendorName || 'the vendor'} and they will be in touch soon.
          </p>
  ...
  return (
    <div style={containerStyle}>
      <h2 style={S.heading}>Get in Touch</h2>
      <div style={S.subtitle}>Send a message to {vendorName || 'this vendor'}</div>
      ...
      <form ref={formRef} onSubmit={handleSubmit}>
          ...
          <div style={S.contactPills}>
            {CONTACT_METHODS.map((method) => (
              <button
                key={method}
                type="button"
                style={{ ...S.pill, ...(form.preferredContact === method ? S.pillActive : {}) }}
                onClick={() => set('preferredContact', form.preferredContact === method ? '' : method)}
              >
                {method}
              </button>
            ))}
          ...
          <div style={S.pillGrid}>
            {BUDGET_OPTIONS.map((label) => (
              <button
                key={label}
                type="button"
                style={{ ...S.pill, ...(form.budgetRange === label ? S.pillActive : {}) }}
                onClick={() => set('budgetRange', form.budgetRange === label ? null : label)}
              >
                {label}
              </button>
            ))}
        ...
        {/* Location */}
        <div style={S.group}>
          <label style={S.label}>Event Location (optional)</label>
          <input type="text" name="locationPreference" value={form.locationPreference} onChange={onChange} placeholder="e.g. London, Cotswolds, South of France" style={S.input} onFocus={focusGold} onBlur={focusReset} />
        </div>
