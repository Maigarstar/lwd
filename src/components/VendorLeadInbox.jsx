...
export default function VendorLeadInbox({ vendorId, C, isMobile }) {
  ...
  const FilterButton = ({ status, label, count }) => (
    <button
      ...
      style={{
        ...
        border: selectedFilter === status ? `1px solid ${C.gold}` : `1px solid ${C.border}`,
        borderRadius: "var(--lwd-radius-input)",
        color: selectedFilter === status ? C.gold : C.grey,
        fontFamily: NU,
        fontSize: 12,
        fontWeight: 600,
  ...
  const EnquiryCard = ({ enquiry }) => (
    <div
      style={{
        background: C.card,
        border: `1px solid ${enquiry.status === "new" ? C.gold + "20" : C.border}`,
      ...
      onMouseLeave={(e) => {
        e.currentTarget.style.borderColor = enquiry.status === "new" ? C.gold + "20" : C.border;
      }}
    >
      {/* Status Indicator */}
      <div
      ...
      <div style={{ flex: 1 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 6 }}>
          <div>
            <p style={{ fontFamily: GD, fontSize: 14, color: C.white, margin: "0 0 2px 0", fontWeight: 600 }}>
              {enquiry.couple_name || "Unknown Couple"}
            </p>
            <p style={{ fontFamily: NU, fontSize: 11, color: C.grey, margin: 0, letterSpacing: "0.5px" }}>
        ...
        {/* Enquiry Details */}
        <p style={{ fontFamily: NU, fontSize: 12, color: C.grey, margin: "6px 0", lineHeight: 1.4 }}>
          {enquiry.message || "No message provided"}
        </p>

        {/* Meta Info */}
        <div style={{ display: "flex", gap: 16, marginTop: 8, fontSize: 11, color: C.grey2 }}>
          {enquiry.guest_count && (
        ...
        {/* Timestamp */}
        <p style={{ fontFamily: NU, fontSize: 10, color: C.grey2, margin: "6px 0 0 0", fontStyle: "italic" }}>
  ...
  return (
    <div style={{ padding: isMobile ? "16px 0" : "20px 0" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <h3 style={{ fontFamily: GD, fontSize: 24, color: C.white, margin: "0 0 12px 0" }}>
          Lead Inbox
        </h3>
        <p style={{ fontFamily: NU, fontSize: 12, color: C.grey, margin: 0, letterSpacing: "0.5px" }}>
