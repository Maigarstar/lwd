...
export default function AuraPellet() {
  ...
  useEffect(() => {
    const on  = () => setCompareBarActive(true);
    const off = () => setCompareBarActive(false);
    window.addEventListener("lwd:compare-bar-open",  on);
    window.addEventListener("lwd:compare-bar-close", off);
    return () => {
      window.removeEventListener("lwd:compare-bar-open",  on);
      window.removeEventListener("lwd:compare-bar-close", off);
    };
  }, []);
