...
import { getRecommendations, loadArticleRecommendations } from "./recommendationEngine";
...
export function ChatProvider({ children }) {
  ...
  // Load article recommendations asynchronously
  useEffect(() => {
    ...
    loadArticleRecommendations(messages)
      .then(({ articles }) => {
        setArticleRecommendations(articles || []);
      })
      .catch((err) => {
        console.error('[ChatContext] loadArticleRecommendations error:', err);
        setArticleRecommendations([]);
      })
      .finally(() => {
  ...
  return (
    <ChatCtx.Provider
      value={{
        ...
        activeContext, recommendations, articleRecommendations, articlesLoading, sessionId: sessionId.current,
